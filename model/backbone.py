...
class BbGCN(MessagePassing):
    ...
    def message(self, x_j: Tensor, drop_rate: float):
        ...
        if not self.training:
            return x_j
        x_j = np.squeeze(x_j, axis=1)
        x_b = x_j * torch.bernoulli(torch.ones(x_j.size(0), 1) -drop_rate).to(x_j.device)
        x_b = x_b / (1 - drop_rate)
        x_b_new = x_b.unsqueeze(1)
        return x_b_new
...
class BbGAT(MessagePassing):
    ...
    def message(self, x_j: Tensor, alpha_j: Tensor, alpha_i: OptTensor, index: Tensor, ptr: OptTensor,
        ...
        x_b = x_b / (1 - drop_rate)
        x_b_new = x_b.unsqueeze(1)
        return x_b_new*alpha.unsqueeze(-1)
